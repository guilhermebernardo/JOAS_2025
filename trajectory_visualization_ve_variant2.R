trajectory_visualization_ve_variant2 <- function(track_data, kpi_data){
  ...
  library("leaflet")
  source('C:/Users/guilherme.bernardo/Desktop/DASC 2025/KPI19/dash_kpi19_micro/get_lat.R')
  source('C:/Users/guilherme.bernardo/Desktop/DASC 2025/KPI19/dash_kpi19_micro/get_lon.R')
  ...
  for (i in unique(track_data$indicat)){
    ...
    if (length(idx) > 0 & length(idx2) > 0){
      ...
      txtlabel <- paste(table_aux$id_icao[1], paste0(round(kpi_data$kpi19_time[idx2],1),' min'), sep=': ')
      #txtlabel <- paste0(paste(table_aux$id_icao[1], paste0(round(kpi_data$kpi19_time[idx2],1),'min'), sep=': '), ' (', date(table_aux$time[1]), ')')
      #m <- addPolylines(m, lng = table_aux$lon, lat = table_aux$lat, color = pal(min(c(kpi_data$kpi19_time[idx2],upper_limit))), weight = 2, opacity = 0.3, label=txtlabel)
      
      for (k in 1:kpi_bins_number){
        if (kpi_data$kpi19_time[idx2[length(idx2)]] >= kpi_bins_lower[k] & kpi_data$kpi19_time[idx2[length(idx2)]] < kpi_bins_upper[k]){
  ...
  #m <- addLegend(m, "bottomright", pal = pal, values = seq(0,upper_limit,0.05), title = "KPI 19 (min)", opacity = 1)
  ...
  m <- addLayersControl(m, position="topleft", baseGroups = c("KPI 19 (min):"), overlayGroups = c(kpi_bins_label), options=layersControlOptions(collapsed=TRUE))
